...
import { Graph, useNode, useLink, LayoutLayer } from 'graphire';
    ...
    <Box>
      <svg style={{ width: '100vw', height: '100vh' }}>
        <Graph>
          ...
          <g id="nodes">
            {graph.nodes.map((e, i) => (
              <Node key={`n-${i}`} uid={e.id} color={colors[e.id % 4]} />
            ))}
          </g>
          {graph.links.map((e, i) => (
            <Link key={`l-${i}`} source={e.source} target={e.target} />
          ))}
...
const Link = (props) => {
  ...
  return <line ref={ref} x1="0" y1="0" x2="0" y2="0" stroke={color} strokeWidth={1} />;
};
