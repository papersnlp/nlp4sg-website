    ...
    <Box column center css={{ width: '100vw', backgroundColor: '$blue9', position: 'relative', zIndex: 1}}>
      <Box container column css={{ py: '$6'}}>
        ...
        <Box column css={{gap: '$3'}}>
          { content.map((event) => <Event key={event.name+event.date} {...event} />)}
        </Box>
      </Box>
    </Box>
  ) 
  ...
  return (
    <Card>     
      <Link href={link} css={{ display: 'flex', flexDirection: 'row'}}>  
        <Box css={{flex: 1, pr: '$3'}}>
          <Text>{name}</Text> 
          <Text mono css={{pt: '$2'}}><TimeAgo date={date} />.</Text>
        </Box>
        <DateBox column center css={{ justifyContent: 'center' }}>
          <Text mono css={{lineHeight: '$3'}}>{monthNames[date.getMonth()]}</Text>
          <DateNum>{date.getDate()}</DateNum>
          <Text mono css={{lineHeight: '$3'}}>{date.getFullYear()}</Text>
        </DateBox>
      </Link>
    </Card>
  )
